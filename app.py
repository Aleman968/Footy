...
def format_dt_short(unix_ts):
    try:
        ...
        dt_local = dt.astimezone(ZoneInfo(APP_TIMEZONE))
        return dt_local.strftime("%d/%m %H:%M")
    except Exception:
        return "-"


def format_day_label(unix_ts):
    try:
        dt = datetime.fromtimestamp(int(unix_ts), tz=timezone.utc)
        dt_local = dt.astimezone(ZoneInfo(APP_TIMEZONE))
        return dt_local.strftime("%A %d/%m")
    except Exception:
        return "Giorno"
...
def evaluate_picks(match, weighted_env=None):
    ...
    pick, reason = choose_best_strategy(p, weighted_env=weighted_env)

    if pick == "NO BET":
        return "NO BET", "Nessuna delle due strategie supera i filtri", p

    return pick, reason, p




def choose_best_strategy(profile, weighted_env=None):
    weighted_env = weighted_env or {}
    mg_total = safe_float(weighted_env.get("mg25", {}).get("weighted_total"), profile.get("mg_weighted_total", 0.0))
    over_total = safe_float(weighted_env.get("over25", {}).get("weighted_total"), profile.get("over_weighted_total", 0.0))

    mg_ok = (
        safe_float(profile.get("over15"), 0.0) > 75 and
        safe_float(profile.get("multigol_24"), 0.0) > 60 and
        safe_float(profile.get("under45"), 0.0) > 78 and
        2.20 <= mg_total <= 3.60
    )

    over_ok = (
        safe_float(profile.get("over25"), 0.0) > 60 and
        1.45 <= safe_float(profile.get("quota_over25"), 0.0) <= 1.70 and
        safe_float(profile.get("quota_gol"), 0.0) > 0 and
        safe_float(profile.get("quota_gol"), 0.0) < safe_float(profile.get("quota_over25"), 0.0) and
        safe_float(profile.get("over05_ht"), 0.0) > 70 and
        over_total >= 2.40
    )

    if not mg_ok and not over_ok:
        return "NO BET", "Nessuna delle due strategie supera i filtri"

    if mg_ok and not over_ok:
        return "MULTIGOL 2-5", f"MG favorito: over15 {profile.get('over15', 0):.0f}%, MG 2-4 {profile.get('multigol_24', 0):.0f}%, under4.5 {profile.get('under45', 0):.0f}%"

    if over_ok and not mg_ok:
        return "OVER 2.5", f"Over favorito: over2.5 {profile.get('over25', 0):.0f}%, over0.5 PT {profile.get('over05_ht', 0):.0f}%, quota {profile.get('quota_over25', 0):.2f}"

    mg_score = (
        safe_float(profile.get("multigol_24"), 0.0) * 0.45 +
        safe_float(profile.get("over15"), 0.0) * 0.25 +
        safe_float(profile.get("under45"), 0.0) * 0.20 +
        max(0.0, 100 - abs(mg_total - 2.8) * 35) * 0.10
    )

    over_score = (
        safe_float(profile.get("over25"), 0.0) * 0.45 +
        safe_float(profile.get("over05_ht"), 0.0) * 0.20 +
        min(over_total / 3.2, 1.0) * 100 * 0.20 +
        (10 if safe_float(profile.get("quota_gol"), 0.0) < safe_float(profile.get("quota_over25"), 999) else 0) +
        min(max((1.70 - safe_float(profile.get("quota_over25"), 1.70)) / 0.36, 0), 1) * 15
    )

    if over_score > mg_score:
        return "OVER 2.5", f"Scelta unica OVER: score Over {over_score:.1f} > score MG {mg_score:.1f}"
    return "MULTIGOL 2-5", f"Scelta unica MG: score MG {mg_score:.1f} >= score Over {over_score:.1f}"
...
def get_grouped_filtered_matches(matches, team_stats_by_league=None, recent_results_by_league=None):
    ...
    for m in matches:
        try:
            ...
            weighted_env = build_weighted_goal_environment(home_stats, away_stats, form_bundle, m)
            pick, reason, profile = evaluate_picks(m, weighted_env=weighted_env)
        except Exception:
            continue
        ...
        kickoff_unix = safe_int(m.get("date_unix") or m.get("match_start_unix"), 0)
        rows.append({
            "strategy": pick,
            "league": league_name,
            "match_name": f"{m.get('home_name', 'Casa')} vs {m.get('away_name', 'Ospite')}",
            "percent": safe_float(profile.get("over25"), 0.0) if pick == "OVER 2.5" else safe_float(profile.get("multigol_24"), 0.0),
            "kickoff_unix": kickoff_unix,
            "kickoff_label": format_dt_short(kickoff_unix),
            "day_label": format_day_label(kickoff_unix),
            "reason": reason,
        })

    rows.sort(key=lambda x: (x["kickoff_unix"], str(x["league"]).lower(), -x["percent"], str(x["match_name"]).lower()))
    return rows
...
def app():
    ...
    st.markdown(
        ...
        .badge-mg{background:linear-gradient(135deg,#50a56d,#3f8d59);}
        .badge-over{background:linear-gradient(135deg,#4b85d1,#3a6eb0);}
        .match-meta{
            font-size:14px;
            color:#6f8096;
            margin-top:10px;
    ...
    if strategy_view != "Dettaglio partita":
        ...
        page_subtitle = "Le migliori partite selezionate per giorno e campionato."
        st.markdown(f'<div class="list-page-title">Partite {target_strategy}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="list-page-subtitle">{page_subtitle}</div>', unsafe_allow_html=True)

        current_day = None
        current_league = None

        for idx, row in enumerate(filtered_rows):
            if row["day_label"] != current_day:
                current_day = row["day_label"]
                current_league = None
                st.markdown(f'<div class="day-header">{current_day.title()}</div>', unsafe_allow_html=True)

            if row["league"] != current_league:
                current_league = row["league"]
                st.markdown(f'<div class="league-header">{current_league}</div>', unsafe_allow_html=True)

            badge_class = "badge-over" if row["strategy"] == "OVER 2.5" else "badge-mg"
            card_html = f"""
            <div class="match-list-card">
                <div class="match-row-top">
                    <div>
                        <div class="match-teams">{row['match_name']}</div>
                        <span class="strategy-badge {badge_class}">Giocata: {row['strategy']}</span>
                        <div class="match-meta">Orario: {row['kickoff_label']}<br>Probabilità: {row['percent']:.0f}%</div>
                    </div>
                    <div class="percent-pill">{row['percent']:.0f}%</div>
                </div>
            </div>
            """
            ...
            with c2:
                if st.button("Apri", key=f"open_match_{target_strategy}_{idx}"):
                    st.session_state["forced_match_label"] = f"{row['kickoff_label']} | {row['match_name']}"
    ...
    pick, reason, profile = evaluate_picks(selected, weighted_env=weighted_env)
    exact_scores = build_exact_score_candidates(selected, profile, form_bundle=form_bundle, team_stats=team_stats)
