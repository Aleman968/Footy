...
def get_google_sheet_worksheet(worksheet_name):
    try:
        service_account_info = dict(st.secrets["gcp_service_account"])
        spreadsheet_id = str(st.secrets["GSHEETS_SPREADSHEET_ID"])
    except Exception:
        st.error("Configurazione Google Sheets mancante. Inserisci gcp_service_account e GSHEETS_SPREADSHEET_ID nei Secrets.")
        st.stop()
    ...
    try:
        ...
        header = [
            "Data", "Campionato", "Partita", "Giocata", "Quota",
            "xG totale", "% Over 2.5", "% MG 2-4", "% Over 1.5", "% Under 4.5"
        ]
        ws.append_row(header)
    return ws


def save_filtered_rows_to_gsheets(rows, worksheet_name):
    if not rows:
        return 0

    ws = get_google_sheet_worksheet(worksheet_name)
    values = []
    for row in rows:
        values.append([
            row.get("date_label", ""),
...
def get_grouped_filtered_matches(matches, team_stats_by_league=None, recent_results_by_league=None):
    ...
    for m in matches:
        ...
        xg_total = round(safe_float(m.get("team_a_xg_prematch"), 0.0) + safe_float(m.get("team_b_xg_prematch"), 0.0), 2)
        quota = safe_float(profile.get("quota_over25"), 0.0) if pick == "OVER 2.5" else safe_float(profile.get("quota_mg_25"), 0.0)
        ...
        rows.append({
            ...
            "date_label": format_dt_short(kickoff_unix).split(" ")[0] if kickoff_unix else "",
            "quota": quota,
            "xg_total": xg_total,
            "over25": safe_float(profile.get("over25"), 0.0),
            "multigol_24": safe_float(profile.get("multigol_24"), 0.0),
            "over15": safe_float(profile.get("over15"), 0.0),
            "under45": safe_float(profile.get("under45"), 0.0),
            "reason": reason,
        })
...
def app():
    ...
    if strategy_view != "Dettaglio partita":
        ...
        with save_col1:
            if st.button(f"💾 Salva tutte su Sheets ({target_strategy})", key=f"save_all_{target_strategy}"):
                worksheet_name = "OVER" if target_strategy == "OVER 2.5" else "MG"
                saved_count = save_filtered_rows_to_gsheets(filtered_rows, worksheet_name)
                st.success(f"Salvate {saved_count} partite nel foglio Google '{worksheet_name}'.")
        with save_col2:
            st.caption("Salva tutto l'elenco filtrato nel foglio Google, poi decidi lì cosa tenere o eliminare.")
        ...
        for idx, row in enumerate(filtered_rows):
            if row["day_label"] != current_day:
                ...
                current_league = None
                st.markdown(f'<div class="day-header">{row["day_label"].title()}</div>', unsafe_allow_html=True)
            ...
            <div class="match-list-card">
                <div class="match-row-top">
                    <div>
                        ...
                        <div class="match-meta">Data: {row['date_label']} · Orario: {row['kickoff_label'].split(' ')[-1]}<br>Probabilità: {row['percent']:.0f}% · xG totale: {row['xg_total']}</div>
                    </div>
