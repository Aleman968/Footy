...
def filter_exact_scores_by_pick(exact_scores, pick):
    filtered = []
    for item in (exact_scores or []):
        scoreline = str(item.get("scoreline", ""))
        try:
            h, a = [int(x) for x in scoreline.split("-")]
            total = h + a
        except Exception:
            filtered.append(item)
            continue

        if "OVER 2.5" in str(pick):
            if total >= 3:
                filtered.append(item)
        elif "MULTIGOL 2-5" in str(pick):
            if 2 <= total <= 5:
                filtered.append(item)
        else:
            filtered.append(item)
...
def app():
    ...
    exact_scores = filter_exact_scores_by_pick(exact_scores, pick)
        ...
        with st.expander("⚽ Mostra statistiche squadre"):
            ...
            with col_stats1:
                ...
                if home_trend.get('count', 0) > 0:
                    st.write(f"Media gol ultime 5: {fmt_stat(home_trend.get('avg_total_goals'))}")
                else:
                    st.write("Media gol ultime 5: -")
            ...
            with col_stats2:
                ...
                if away_trend.get('count', 0) > 0:
                    st.write(f"Media gol ultime 5: {fmt_stat(away_trend.get('avg_total_goals'))}")
                else:
                    st.write("Media gol ultime 5: -")
            ...
            with sum1:
                st.write(f"**xG Totale:** {fmt_stat(xg_total)}")
            with sum2:
                st.write(f"**Gol attesi MG:** {fmt_stat(weighted_env.get('mg25', {}).get('expected_home_goals'))} / {fmt_stat(weighted_env.get('mg25', {}).get('expected_away_goals'))}")
            with sum3:
                st.write(f"**Gol attesi Over:** {fmt_stat(weighted_env.get('over25', {}).get('expected_home_goals'))} / {fmt_stat(weighted_env.get('over25', {}).get('expected_away_goals'))}")

            with st.expander("🔍 Mostra dati avanzati del modello"):
                adv1, adv2 = st.columns(2)
                with adv1:
                    st.markdown(f"**{selected.get('home_name', 'Casa')}**")
                    st.write(f"Gol subiti in casa: {fmt_stat(home_stats.get('conceded_home'))}")
                    if home_trend.get('count', 0) > 0:
                        st.write(f"GG ultime 5: {home_trend.get('gg_count', 0)}/{home_trend.get('count', 0)}")
                        st.write(f"Over 2.5 ultime 5: {home_trend.get('over25_count', 0)}/{home_trend.get('count', 0)}")
                        st.write(f"Under 4.5 ultime 5: {home_trend.get('under45_count', 0)}/{home_trend.get('count', 0)}")
                    st.write(f"Attacco ponderato MG: {fmt_stat(weighted_env.get('mg25', {}).get('home', {}).get('scored_weighted'))}")
                    st.write(f"Difesa ponderata MG: {fmt_stat(weighted_env.get('mg25', {}).get('home', {}).get('conceded_weighted'))}")
                    st.write(f"Attacco ponderato Over: {fmt_stat(weighted_env.get('over25', {}).get('home', {}).get('scored_weighted'))}")
                    st.write(f"Difesa ponderata Over: {fmt_stat(weighted_env.get('over25', {}).get('home', {}).get('conceded_weighted'))}")

                with adv2:
                    st.markdown(f"**{selected.get('away_name', 'Ospite')}**")
                    st.write(f"Gol subiti in trasferta: {fmt_stat(away_stats.get('conceded_away'))}")
                    if away_trend.get('count', 0) > 0:
                        st.write(f"GG ultime 5: {away_trend.get('gg_count', 0)}/{away_trend.get('count', 0)}")
                        st.write(f"Over 2.5 ultime 5: {away_trend.get('over25_count', 0)}/{away_trend.get('count', 0)}")
                        st.write(f"Under 4.5 ultime 5: {away_trend.get('under45_count', 0)}/{away_trend.get('count', 0)}")
                    st.write(f"Attacco ponderato MG: {fmt_stat(weighted_env.get('mg25', {}).get('away', {}).get('scored_weighted'))}")
                    st.write(f"Difesa ponderata MG: {fmt_stat(weighted_env.get('mg25', {}).get('away', {}).get('conceded_weighted'))}")
                    st.write(f"Attacco ponderato Over: {fmt_stat(weighted_env.get('over25', {}).get('away', {}).get('scored_weighted'))}")
                    st.write(f"Difesa ponderata Over: {fmt_stat(weighted_env.get('over25', {}).get('away', {}).get('conceded_weighted'))}")
    ...
    with col2:
        ...
        if standings_rows:
            ...
            for row in standings_rows[:20]:
                ...
                team_label = f"<strong>{row['Pos']}. {row['Squadra']}{marker_star}</strong>" if is_selected else f"{row['Pos']}. {row['Squadra']}{marker_star}"
                points_label = f"<strong>{row['Pt']} pt</strong>" if is_selected else f"{row['Pt']} pt"
                class_html += f'<div class="standing-row"><span class="{team_class}">{team_label}</span><span class="{team_class}">{points_label}</span></div>'
